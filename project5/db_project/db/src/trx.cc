...
lock_t*
give_lock(uint64_t bitmap, int trx_id, bool lock_mode)
{
  lock_t*                 lock;
...
deadlock_detect(int trx_id) 
{
  ...
  LOCK(trx_mutex);
  std::vector<int> visit(trx_manager->trx_cnt + 5);
  visit[trx_id] = 1;
  target_id = trx_manager->trx_table[trx_id-1]->wait_trx_id;
  while(1) {
    if(!target_id) {
      flag = false;
      break;
    ...
    visit[target_id] = 1;
    target_id = trx_manager->trx_table[target_id-1]->wait_trx_id;
  }
  UNLOCK(trx_mutex);
...
db_find(int64_t table_id, int64_t key, char* ret_val, uint16_t *val_size, int trx_id)
{
  ...
  flag = lock_acquire(table_id, page_id, key_index, trx_id, SHARED);
  if(flag == DEADLOCK) {
    trx_abort(trx_id);
    return 1;
  }
...
db_update(int64_t table_id, int64_t key, char* values, uint16_t new_val_size, uint16_t* old_val_size, int trx_id)
{
  ...
  flag = lock_acquire(table_id, page_id, key_index, trx_id, EXCLUSIVE);
  if(flag == DEADLOCK) {
    trx_abort(trx_id);
    return 1;
  }
...
int
lock_acquire(int64_t table_id, pagenum_t page_id, int kindex, int trx_id, bool lock_mode)
{
  entry_t*      entry;
  lock_t*       point;
  lock_t*       new_lock;
  ...
  bitmap = MASK(kindex);
  trx = trx_manager->trx_table[trx_id-1];
  new_lock = give_lock(bitmap, trx_id, lock_mode);
