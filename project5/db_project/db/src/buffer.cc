...
int hit_idx(int64_t table_id, pagenum_t pagenum) {
    ...
    i = hashValue;
    while(1) {
        if(frames[i].is_buf && frames[i].table_id==table_id && frames[i].page_num==pagenum) {
            delete_append_LRU(i);
            return i;
        }
        i = (i+1)%tablesize;
...
pagenum_t buffer_alloc_page(int64_t table_id) {
    ...
    if(hit>=0) {
        ...
        if(!frames[hit].page->nextfree_num) {
            ...
            if(num_frames < num_bufs) new_idx = find_empty_frame(table_id, new_pagenum);
    ...
    if(num_frames<num_bufs) hit = find_empty_frame(table_id, new_pagenum);
    else hit = give_idx();
    frames[hit].state = LOCKED;
    LOCK(frames[hit].page_mutex);
...
page_t* buffer_read_page(int64_t table_id, pagenum_t pagenum, int* idx, bool mode) {
    ...
    if(num_frames<num_bufs) hit = find_empty_frame(table_id, pagenum);
    else hit = give_idx();
