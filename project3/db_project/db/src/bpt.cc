...
pagenum_t find_leaf(int64_t table_id, pagenum_t root_num, int64_t key) {
    ...
    page_t* root;
    root_idx = buffer_get_idx(table_id, root_num);
    root = buffer_read_page(table_id, root_num, root_idx);
    manage_pin(root_idx, 1);
    ...
    while(!root->info.isLeaf) {
        ...
        next_idx = buffer_get_idx(table_id, ret_num);
        root = buffer_read_page(table_id, ret_num, next_idx);
        manage_pin(next_idx, 1);
        manage_pin(root_idx, 0);
        root_idx = next_idx;
    }
    manage_pin(root_idx, 0);
...
int db_find(int64_t table_id, int64_t key, char * ret_val, uint16_t * val_size) {
    ...
    header_idx = buffer_get_idx(table_id, 0);
    header = buffer_read_page(table_id, 0, header_idx);
    root_num = header->root_num;
    manage_pin(header_idx, 1);
    if(!root_num) return 1;

    leaf_num = find_leaf(table_id, root_num, key);
    leaf_idx = buffer_get_idx(table_id, leaf_num);
    leaf = buffer_read_page(table_id, leaf_num, leaf_idx);
    manage_pin(header_idx, 0);
    manage_pin(leaf_idx, 1);
...
int insert_into_internal(int64_t table_id, uint32_t index, pagenum_t parent_num, page_t *parent, int32_t parent_idx, pagenum_t l_num, page_t* l, int32_t l_idx, pagenum_t r_num, page_t* r, int32_t r_idx, int64_t key) {
    ...
    buffer_write_page(table_id, l_num, l_idx);
    buffer_write_page(table_id, r_num, r_idx);
    buffer_write_page(table_id, parent_num, parent_idx);

    manage_pin(l_idx, 0);
    manage_pin(r_idx, 0);
    manage_pin(parent_idx, 0);
...
int insert_into_internal_after_splitting(int64_t table_id, uint32_t index, pagenum_t parent_num, page_t *parent, int32_t parent_idx, pagenum_t r_num, int64_t key) {
    ...
    int32_t new_parent_idx, child_idx, child2_idx;
    ...
    new_parent_idx = buffer_get_idx(table_id, new_parent_num);
    new_parent = buffer_read_page(table_id, new_parent_num, new_parent_idx);
    manage_pin(new_parent_idx, 1);
    ...
    child_idx = buffer_get_idx(table_id, new_parent->leftmost);
    child = buffer_read_page(table_id, new_parent->leftmost, child_idx);
    manage_pin(child_idx, 1);
    child->parent_num = new_parent_num;
    buffer_write_page(table_id, new_parent->leftmost, child_idx);

    for(uint32_t i=split+1, j=0; i<num_keys; i++, j++) {
        new_parent->branch[j].key = tmp[i].key;
        new_parent->branch[j].pagenum = tmp[i].pagenum;

        child2_idx = buffer_get_idx(table_id, tmp[i].pagenum);
        child = buffer_read_page(table_id, tmp[i].pagenum, child2_idx);
        manage_pin(child2_idx, 1);
        child->parent_num = new_parent_num;
        buffer_write_page(table_id, tmp[i].pagenum, child2_idx);

        manage_pin(child_idx, 0);
...
int insert_into_parent(int64_t table_id, pagenum_t l_num, page_t* l, int32_t l_idx, pagenum_t r_num, page_t* r, int32_t r_idx, int64_t key) {
    ...
    if(!parent_num) {
        ...
        new_root_idx = buffer_get_idx(table_id, new_root_num);
        new_root = buffer_read_page(table_id, new_root_num, new_root_idx);
        manage_pin(new_root_idx, 1);

        header_idx = buffer_get_idx(table_id, 0);
        header = buffer_read_page(table_id, 0, header_idx);
        header->root_num = new_root_num;
        buffer_write_page(table_id, 0, header_idx);
        ...
        buffer_write_page(table_id, l_num, l_idx);
        buffer_write_page(table_id, r_num, r_idx);
        buffer_write_page(table_id, new_root_num, new_root_idx);

        manage_pin(l_idx, 0);
        manage_pin(r_idx, 0);
        manage_pin(new_root_idx, 0);
        return 0;
    }
    parent_idx = buffer_get_idx(table_id, parent_num);
    parent = buffer_read_page(table_id, parent_num, parent_idx);
    manage_pin(parent_idx, 1);
...
int insert_into_leaf(int64_t table_id, uint32_t index, pagenum_t leaf_num, page_t* leaf, int leaf_idx, int64_t key, char * value, uint16_t val_size) {
    ...
    buffer_write_page(table_id, leaf_num, leaf_idx);
    manage_pin(leaf_idx, 0);
    return 0;
}
...
int insert_into_leaf_after_splitting(int64_t table_id, uint32_t index, pagenum_t leaf_num, page_t* leaf, int leaf_idx, int64_t key, char* value, uint16_t val_size) {
    ...
    new_leaf_idx = buffer_get_idx(table_id, new_leaf_num);
    new_leaf = buffer_read_page(table_id, new_leaf_num, new_leaf_idx);
    manage_pin(new_leaf_idx, 1);
...
int start_new_tree(int64_t table_id, int64_t key, char * value, uint16_t val_size) {
    ...
    new_root_num = buffer_alloc_page(table_id);
    root_idx = buffer_get_idx(table_id, new_root_num);
    new_root = buffer_read_page(table_id, new_root_num, root_idx);
    manage_pin(root_idx, 1);
    ...
    buffer_write_page(table_id, new_root_num, root_idx);
    manage_pin(root_idx, 0);
    return 0;
}
...
int db_insert(int64_t table_id, int64_t key, char * value, uint16_t val_size) {
    ...
    if(leaf->freespace>=12+val_size) return insert_into_leaf(table_id, i, leaf_num, leaf, leaf_idx, key, value, val_size);
...
int get_my_index(int64_t table_id, pagenum_t pagenum, page_t* page) {
    ...
    parent_idx = buffer_get_idx(table_id, page->parent_num);
    parent = buffer_read_page(table_id, page->parent_num, parent_idx);
    manage_pin(parent_idx, 1);
...
int adjust_root(int64_t table_id, pagenum_t root_num, page_t* root, int32_t root_idx, int64_t key) {
    ...
    if(!root->info.num_keys) {
        ...
        if(root->info.isLeaf) {
            ...
            new_root_idx = buffer_get_idx(table_id, root->leftmost);
            new_root = buffer_read_page(table_id, root->leftmost, new_root_idx);
            manage_pin(new_root_idx, 1);
            new_root->parent_num = 0;
            buffer_write_page(table_id, root->leftmost, new_root_idx);
            manage_pin(new_root_idx, 0);
        }
        buffer_write_page(table_id, 0, header_idx);
        manage_pin(header_idx, 0);
        ...
        buffer_write_page(table_id, root_num, root_idx);
        manage_pin(root_idx, 0);
    }
...
int coalesce_leaf(int64_t table_id, int my_index, pagenum_t parent_num, page_t* parent, int32_t parent_idx, pagenum_t sibling_num, page_t* sibling, int32_t sibling_idx, pagenum_t leaf_num, page_t* leaf, int32_t leaf_idx) {
    ...
    buffer_write_page(table_id, sibling_num, sibling_idx);
    manage_pin(sibling_idx, 0);
...
int coalesce_internal(int64_t table_id, int my_index, pagenum_t parent_num, page_t* parent, int32_t parent_idx, pagenum_t sibling_num, page_t* sibling, int32_t sibling_idx, pagenum_t page_num, page_t* page, int32_t page_idx) {
    ...
    child_idx = buffer_get_idx(table_id, page->leftmost);
    child = buffer_read_page(table_id, page->leftmost, child_idx);
    manage_pin(child_idx, 1);
    child->parent_num = sibling_num;
    buffer_write_page(table_id, page->leftmost, child_idx);
    ...
    for(int j=0; j<num_keys; j++, i++) {
        ...
        child2_idx = buffer_get_idx(table_id, sibling->branch[i].pagenum);
        child = buffer_read_page(table_id, sibling->branch[i].pagenum, child2_idx);
        manage_pin(child2_idx, 1);
        child->parent_num = sibling_num;
        buffer_write_page(table_id, sibling->branch[i].pagenum, child2_idx);
...
int redistribute_internal(int64_t table_id, pagenum_t parent_num, page_t* parent, int32_t parent_idx, pagenum_t sibling_num, page_t* sibling, int32_t sibling_idx, pagenum_t page_num, page_t* page, int32_t page_idx, int my_index) {
    ...
    if(my_index==-1) {    
        ...
        child_idx = buffer_get_idx(table_id, sibling->leftmost);
        child = buffer_read_page(table_id, sibling->leftmost, child_idx);
        manage_pin(child_idx, 1);
        child->parent_num = page_num;
        buffer_write_page(table_id, sibling->leftmost, child_idx);
        manage_pin(child_idx, 0);
        ...
        child_idx = buffer_get_idx(table_id, page->branch[0].pagenum);
        child = buffer_read_page(table_id, page->branch[0].pagenum, child_idx);
        manage_pin(child_idx, 1);
        child->parent_num = page_num;
        buffer_write_page(table_id, page->branch[0].pagenum, child_idx);
        manage_pin(child_idx, 0);
    ...
    buffer_write_page(table_id, sibling_num, sibling_idx);
    buffer_write_page(table_id, page_num, page_idx);
    buffer_write_page(table_id, parent_num, parent_idx);

    manage_pin(sibling_idx, 0);
    manage_pin(page_idx, 0);
    manage_pin(parent_idx, 0);
...
int delete_entry(int64_t table_id, pagenum_t page_num, page_t* page, int32_t page_idx, int64_t key) {
    ...
    if(page->info.isLeaf) {
        ...
        sibling_idx = buffer_get_idx(table_id, sibling_num);
        sibling = buffer_read_page(table_id, sibling_num, sibling_idx);
        manage_pin(sibling_idx, 1);
