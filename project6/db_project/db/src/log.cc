...
void
open_recovery_table(int64_t table_id)
{
	char path[10];
...
void
push_log_to_buffer(main_log_t* main_log, update_log_t* update_log, char* old_img, char* new_img, LSN_t next_undo_LSN)
{
	uint16_t valsize;
...
main_log_t*
make_main_log(int trx_id, int type, int log_size, LSN_t prev_LSN)
{
	main_log_t* main_log;
...
update_log_t*
make_update_log(int64_t table_id, pagenum_t page_id, uint16_t valsize, uint16_t offset)
{
...
void
make_active_trx(int trx_id, LSN_t first_LSN)
{
...
void
push_update_stack(int trx_id, LSN_t LSN) 
{
	active_trx_t* active_trx;
	if(activetrans.find(trx_id) == activetrans.end()) return;
	active_trx = activetrans[trx_id];
...
void
pop_update_stack(int trx_id) 
{
	active_trx_t* active_trx;
	if(activetrans.find(trx_id) == activetrans.end()) return;
	active_trx = activetrans[trx_id];
	if(!active_trx->update_LSN_stack.empty())
		active_trx->update_LSN_stack.pop();
}

LSN_t
top_update_stack(int trx_id) 
{
	active_trx_t* active_trx;
	LSN_t ret;
...
redo(int log_num)
{
	...
	while((LSN < header_log->flushed_LSN) && (log_num == -1 || ++loop<=log_num))
	{
		...
		if(main_log->type == UPDATE || main_log->type == COMPENSATE) 
		{
			...
			if(main_log->type == UPDATE) push_update_stack(main_log->trx_id, LSN);
			else {
			...
			if(page->LSN < LSN) {
				...
				if(main_log->type == UPDATE) fprintf(logmsgFP, "LSN %lu [UPDATE] Transaction id %d redo apply\n", LSN, main_log->trx_id);					
				else fprintf(logmsgFP, "LSN %lu [CLR] next undo lsn %lu\n", LSN, next_undo_LSN);
		...
		{
			if(main_log->type == BEGIN) fprintf(logmsgFP, "LSN %lu [BEGIN] Transaction id %d\n", LSN, main_log->trx_id);
			else if(main_log->type == COMMIT)  fprintf(logmsgFP, "LSN %lu [COMMIT] Transaction id %d\n", LSN, main_log->trx_id);
			else fprintf(logmsgFP, "LSN %lu [ROLLBACK] Transaction id %d\n", LSN, main_log->trx_id);
		}
		LSN += main_log->log_size;
	}
...
undo(int log_num)
{
	...
	for(int trx_id : loser) {
		...
		else priority_table.push(top_update_stack(trx_id));
	}
	...
	{
		LSN = priority_table.top(); priority_table.pop();
		...
		if(page->LSN >= LSN) 
		{
			new_main_log = make_main_log(trx_id, COMPENSATE, MAINLOG + UPDATELOG + 2*size + 8, activetrans[trx_id]->last_LSN);
			new_update_log = make_update_log(table_id, page_id, size, offset + 128);
			activetrans[trx_id]->last_LSN = new_main_log->LSN;
			push_log_to_buffer(new_main_log, new_update_log, new_img, old_img, next_undo_LSN);
